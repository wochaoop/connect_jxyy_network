...
$(gz_releases): %.gz : %
	mkdir -p build_tmp/$(basename $@) # Create a temporary directory
	GOARCH=$(subst -, ,$(word 2,$(subst /, ,$@))) GOOS=$(subst -, ,$(word 1,$(subst /, ,$@))) $(GOBUILD) -o build_tmp/$(basename $@)/$(NAME)
	cp config_example.yaml build_tmp/$(basename $@)/config.yaml # Copy config.yaml
	gzip -f -S -$(VERSION).gz -c build_tmp/$(basename $@) > $(BINDIR)/$(NAME)-$(basename $@).gz
	rm -r build_tmp/$(basename $@) # Remove the temporary directory

$(zip_releases): %.zip : %
	mkdir -p build_tmp/$(basename $@) # Create a temporary directory
	GOARCH=$(subst -, ,$(word 2,$(subst /, ,$@))) GOOS=$(subst -, ,$(word 1,$(subst /, ,$@))) $(GOBUILD) -o build_tmp/$(basename $@)/$(NAME).exe
	cp config_example.yaml build_tmp/$(basename $@)/config.yaml # Copy config.yaml
	zip -j $(BINDIR)/$(NAME)-$(basename $@)-$(VERSION).zip build_tmp/$(basename $@)/*
	rm -r build_tmp/$(basename $@) # Remove the temporary directory
